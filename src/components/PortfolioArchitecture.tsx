...
const CONTENT: ArchitectureContent = {
    ...
    dataFlow:
        "Plaid is the only runtime source: the Express /api/holdings route reshapes raw Plaid holdings into privacy-safe rows — ticker, relative weight, and cost-basis return, never dollar amounts — cached server-side (6h) with in-flight dedup and serve-stale-on-error. Live daily quotes come from Finnhub (/api/quotes, 2-min cache). Everything heavy is baked at schedule time: a weekly GitHub Actions job pulls ~6 years of dividend-adjusted prices + dividend events from Yahoo (plus SPY as the S&P 500 benchmark) and per-holding metrics/profiles/analyst data from Finnhub (throttled ≤1 req/s), commits the JSON, and rsyncs it live. The React app reads those static files (cache-revalidated) and computes every return on the page — chart, risk-card 1Y, sparkline, per-position total return — from the same baked adjusted closes, so widgets never disagree. Per-visit third-party API cost is ≈ zero.",
 ...
 GitHub Actions — weekly bake
   Yahoo   → 6y adj. prices + dividend events (+ SPY benchmark)
   ...
   └─► holdingsHistory.json + holdingsMeta.json ─► commit + rsync to prod`,
    decisions: [
        ...
        { title: "Free-tier safeguards", body: "Per-visit cost is bounded by server caches (holdings 6h, quotes 2-min) with in-flight dedup + serve-stale-on-error; all historical/metadata data is baked weekly into committed JSON, with Finnhub calls throttled to ≤1 req/s so the bake itself never trips a rate limit." },
